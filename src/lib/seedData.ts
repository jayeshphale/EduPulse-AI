...
export async function seedDatabase() {
  // On Vercel, ensure SQLite database exists in /tmp/dev.db
  if (process.env.VERCEL) {
    try {
      ...
      if (!fs.existsSync(tmpPath) && fs.existsSync(sourcePath)) {
        ...
        console.log('Copied seed SQLite database to /tmp/dev.db on Vercel');
      }
    } catch (e) {
      console.warn('Vercel tmp copy notice:', e);
    }
  }
  ...
  try {
    ...
    console.warn('Database count query failed, re-seeding:', err);
  }
